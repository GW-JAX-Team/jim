...
def _tiny_flowmc_config(**kwargs) -> FlowMCSamplerConfig:
    """Minimal FlowMCSamplerConfig for fast unit tests."""
    defaults = dict(
        n_chains=5,
        n_local_steps=2,
        n_global_steps=2,
    ...
    return FlowMCSamplerConfig(**defaults)
...
@pytest.fixture
def jim_with_likelihood_transforms(mock_likelihood, gw_prior, mass_ratio_to_eta_transform):
    return Jim(
        likelihood=mock_likelihood,
        prior=gw_prior,
        sampler_config=_tiny_flowmc_config(),
...
def jim_sampler(mock_likelihood, gw_prior, monkeypatch):
    ...
    n_samples = 20
    mock_samples = {
        "M_c": np.ones(n_samples) * 30.0,
        "q": np.ones(n_samples) * 0.5,
    }
    mock_training_samples = {
        "M_c": np.ones(n_samples) * 25.0,
        "q": np.ones(n_samples) * 0.4,
    }
    mock_output = SamplerOutput(
        samples=mock_samples,
        log_posterior=np.ones(n_samples) * -2.0,
        metadata={
            "training_samples": mock_training_samples,
            "training_log_posterior": np.ones(n_samples) * -3.0,
            "loss_history": np.zeros(10),
            "local_accs": np.ones((10, 2)),
            "global_accs": np.ones((10, 2)),
        },
    )
...
class TestGetSamples:
    ...
    def test_get_samples_training_vs_production(self, jim_sampler):
        training = jim_sampler.get_samples(training=True)
        production = jim_sampler.get_samples(training=False)
        assert training.keys() == production.keys()
        for key in training:
            assert isinstance(training[key], np.ndarray)
            assert isinstance(production[key], np.ndarray)
    ...
    def test_get_samples_with_sample_transforms(
        ...
        n = 10
        mock_output = SamplerOutput(
            samples={"M_c": np.ones(n) * 30.0, "q": np.ones(n) * 0.5},
            log_posterior=np.ones(n) * -2.0,
            metadata={
                "training_samples": {"M_c": np.ones(n) * 25.0, "q": np.ones(n) * 0.4},
                "training_log_posterior": np.ones(n) * -3.0,
            },
        )
        monkeypatch.setattr(jim.sampler, "get_output", lambda: mock_output)
        jim.sampler._sampled = True
    ...
    def test_get_samples_warning_when_requesting_more_than_available(
        ...
        assert any(
            "Requested 100 samples" in r.message and f"only {n_available} available" in r.message
...
class TestJimTempering:
    def test_with_tempering_enabled(self, mock_likelihood, gw_prior):
        jim = Jim(
            likelihood=mock_likelihood,
            prior=gw_prior,
            sampler_config=_tiny_flowmc_config(n_temperatures=3),
        )
    ...
    def test_with_tempering_disabled(self, mock_likelihood, gw_prior):
        jim = Jim(
            likelihood=mock_likelihood,
            prior=gw_prior,
            sampler_config=_tiny_flowmc_config(n_temperatures=0),
        )
...
class TestJimPosteriorEvaluation:
    ...
    def test_evaluate_posterior_with_sample_transforms(self, jim_with_sample_transforms):
        assert "M_c_unbounded" in jim_with_sample_transforms.parameter_names
        samples_transformed = jnp.array([0.5, 0.6])
        assert jnp.isfinite(jim_with_sample_transforms.evaluate_posterior(samples_transformed))
